...
import com.lcy.aigc.amqp.MyMessageProducer;
...
//            ThrowUtils.throwIf(split.length<3,ErrorCode.SYSTEM_ERROR,"Ai响应错误");
    ...
    public BaseResponse<AiResponse> genChartAsyncByAiMq(@RequestPart("file") MultipartFile file,
                                                      GenChartByAiRequest genChartByAiRequest, HttpServletRequest request) {
        String name = genChartByAiRequest.getName();
        String goal = genChartByAiRequest.getGoal();
        String chartType = genChartByAiRequest.getChartType();
        ...
        //给消息队列发消息（消息为保存用户输入之后的id记录）
        myMessageProducer.sendMessage(String.valueOf(id));
        //返回
        AiResponse aiResponse = new AiResponse();
        return ResultUtils.success(aiResponse);
    }
